...
var FirebasePassportLogin = (function (ref, callback, oAuthServerURL) {
  ...
  /**
   * Authenticate to Firebase with the custom token and store it as a `passportSession` cookie.
   * Then call the user-provided callback method with the error code (if any) and auth data.
   * @param token - Passport session token
   * @private
   */
  self._setToken = function (token) {
    if (!token) return;
    document.cookie = 'passportSession=' + token + '; path=/';    // what is this for?
    self._ref.authWithCustomToken(token, function (error, authData) {
      var auth = authData ? authData.auth : {};
  ...
  self.login = function (provider) {
    ...
    }, {remember: 'none'});   // remember: 'none', 'sessionOnly', 'default'
  };
  ...
  // The following code provides the cookie object used to get/set HTTP cookies
...
//module.exports = FirebasePassportLogin;
